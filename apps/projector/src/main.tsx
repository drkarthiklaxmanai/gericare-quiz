...
function App(){
 ...
 useEffect(()=>{if(!supabase)return;const load=async()=>{const{data}=await supabase.from('presentation_state').select('*').order('updated_at',{ascending:false}).limit(1).maybeSingle();if(data)setView(data as DisplayState)};void load();const ch=supabase.channel('projector-display').on('postgres_changes',{event:'*',schema:'public',table:'presentation_state'},p=>{if(p.new)setView(p.new as DisplayState)}).subscribe(s=>setConnected(s==='SUBSCRIBED'));return()=>{supabase.removeChannel(ch)}},[]);
 const board=normalizeBoard(view.top10);const options=normalizeOptions(view.options);const hasCorrect=options.some(o=>isCorrect(o,view.answer));const showRoundCol=view.state==='ROUND_TOP10'&&board.some(b=>b.round_score!=null);
 const content=()=>{switch(view.state){
 case'RULES':return <div className="stage"><div className="kicker">Rules</div><h1>How to play</h1><ul className="rules"><li><span>1</span> Join on your phone or tablet</li><li><span>2</span> Answer before the timer ends</li><li><span>3</span> One device · fair play</li><li><span>4</span> Top scores advance</li></ul></div>;
 case'QUESTION':{const liveOnly=!options.length;return <div className="stage"><div className="kicker">{liveOnly?'Live':(view.title??(view.round_number?`Round ${view.round_number}`:'Question'))}</div>{liveOnly?<div className="hero-card"><h1 style={{whiteSpace:'pre-line',fontSize:'clamp(32px,5vw,72px)'}}>{view.question}</h1><p className="sub" style={{marginTop:20}}><span className="pulse"/>Devices open — answer now</p></div>:<><h2 style={{whiteSpace:'pre-line'}}>{view.question}</h2><MediaBlock view={view}/><div className="options">{options.map((o,i)=><div key={i} className="opt"><span className="key">{o.key}</span><span>{o.text}</span></div>)}</div></>}</div>}
 ...
 case'ROUND_TOP10':case'LEADERBOARD':return <div className="stage"><div className="kicker gold">{view.state==='ROUND_TOP10'?'Round top 10':'Overall standings'}</div><h1 style={{fontSize:'clamp(28px,4.5vw,56px)'}}>{view.title??'Leaderboard'}</h1><div className={'board'+(showRoundCol?' with-round':'')}><div className="board-head"><span>#</span><span></span><span>Name</span>{showRoundCol&&<span className="num">Round</span>}<span className="num">Total</span></div>{board.length?board.map(x=>{const cls=x.rank===1?' top':x.rank===2?' top2':x.rank===3?' top3':'';return <div className={'board-row'+cls} key={`${x.rank}-${x.name}`}><span className="rank">#{x.rank}</span><span className="move">{moveEl(x.rank_delta,x.prev_rank)}</span><span className="name">{x.name}</span>{showRoundCol&&<span className="pts round">{x.round_score??0}</span>}<span className="pts">{x.total_score??x.score}</span></div>}):<div className="empty">Standings will appear after results are released</div>}</div></div>;
 case'FINAL':return <div className="stage"><div className="kicker violet">Grand Final</div><div className="hero-card"><h1>{view.question??view.title??'Final'}</h1><p className="sub">Top teams · final questions</p></div></div>;
