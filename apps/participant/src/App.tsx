...
export default function App(){
 ...
 {screen==='home'&&<><section className="card hero"><p className="eyebrow">PARTICIPANT</p><h1>Welcome{participant?.display_name?`, ${participant.display_name}`:''}</h1><p>Your 10-minute timer starts independently when you press Start.</p>{message&&<div className="notice">{message}</div>}<div className="nav"><button onClick={showHistory}>My Quiz</button><button className="secondary" onClick={showLeaderboard}>Leaderboard</button></div></section>{event&&<FinalPanel eventId={event.id}/>}<section className="roundList">{rounds.map(r=>{const a=attemptFor(r.id);const canStart=r.status==='open'&&!a;return <div className="card roundCard" key={r.id}><div><p className="eyebrow">ROUND {r.round_number}{r.is_optional?' · OPTIONAL':''}</p><h3>{r.title}</h3><p>{a?`Status: ${a.status}`:r.status==='open'?'Open now':r.status}</p></div><button disabled={!canStart||busy} onClick={()=>start(r)}>{a?'Attempt used':canStart?'Start':'Unavailable'}</button></div>})}</section>{session&&<button className="secondary standalone" onClick={signOut}>Sign out</button>}</>}
 ...
 {screen==='submitted'&&<section className="card hero"><div className="check">✓</div><p className="eyebrow">SUBMITTED</p><h1>Round submitted</h1><p>Your answers and score stay hidden until the 15-minute release window opens.</p><button onClick={()=>setScreen('home')}>Back to Home</button></section>}
