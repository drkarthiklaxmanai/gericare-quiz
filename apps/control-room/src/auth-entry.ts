...
async function authorize(){
 ...
 authInFlight=(async()=>{
  try{
   ...
   if(!access){
    shell('Access denied','<p>This account is signed in but does not have Control Room privileges.</p><button id="logout" style="padding:10px 14px;border:0;border-radius:9px;background:#172033;color:#fff;font-weight:700">Sign out</button>');
   ...
   try{
    ...
    shell('Control Room failed to load',`<p style="color:#b91c1c">${msg}</p><button id="retry" style="padding:10px 14px;border:0;border-radius:9px;background:#172033;color:#fff;font-weight:700">Retry</button>`);
   ...
   appLoaded=false;
   const msg=e instanceof Error?e.message:String(e);
   shell('Control Room',`<p>Unexpected error: ${msg}</p>`);
  }
