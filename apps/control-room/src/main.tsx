...
function App(){
  ...
  useEffect(()=>{
    ...
    const tick=()=>{void rpc('acquire_control_lock',{p_event_id:eid}).catch(()=>{})};
  ...
  const control=(r:Round)=>action('Acquiring live control',async()=>{
    await rpc('acquire_control_lock',{p_event_id:r.event_id});
    lockEventRef.current=r.event_id;
    setSelected(r);
    setLocked(true);
  });

  const open=(r:Round)=>action('Opening round',async()=>{
    const otherOpen=rounds.find(x=>x.status==='open'&&x.id!==r.id);
    if(otherOpen)throw Error(`Close Round ${otherOpen.round_number} first (only one round can be open).`);
    if(!r.questions_locked)throw Error('Lock 3 approved questions in Admin before opening this round.');
    await rpc('open_round',{p_event_id:r.event_id,p_round_id:r.id});
  });

  const close=(r:Round)=>action('Closing round',()=>rpc('close_round',{p_event_id:r.event_id,p_round_id:r.id}));

  const release=(r:Round)=>action('Releasing results',async()=>{
  ...
  const startSudden=()=>action('Starting sudden death',async()=>{
  ...
  const resolveSudden=()=>action('Resolving sudden death',async()=>{
