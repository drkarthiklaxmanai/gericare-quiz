          ...
          <>
            ...
            <section className="section">
              ...
              <p className="muted" style={{marginBottom:10}}>
                Clears all attempts, scores, integrity flags, leaderboards and finalists.
                Keeps questions, round sets, and registered participants. Rounds go back to <b>draft</b>.
              </p>
              <div className="actions">
                <button type="button" className="btn danger" onClick={()=>void run('Reset participation',async()=>{
                  if(!eventId)throw Error('No event');
                  if(!window.confirm('Reset ALL participation and scores for this event?\n\nThis cannot be undone. Questions and round line-ups are kept.'))return;
                  if(!window.confirm('Final confirmation: wipe attempts, scores and leaderboards so the quiz can be run again?'))return;
                  const data=await rpc('reset_event_participation',{p_event_id:eventId});
                  setBoard([]);setPrevRanks(new Map());setFinalists([]);setIntegrity([]);setSudden(null);setPlaylist([]);setPlayIdx(0);
                  await publish('WAITING',{p_title:'GeriCare Conference Quiz'});
                  setMessage('Participation reset — '+JSON.stringify(data??{}));
                })}>Reset participation & scores</button>
              </div>
            </section>
          </>
        )}
