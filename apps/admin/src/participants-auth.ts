...
async function authorize(){
 ...
 authInFlight=(async()=>{const{data:{user},error}=await supabase.auth.getUser();if(error||!user){renderLogin();return}let q=supabase.from('event_admins').select('event_id,role').eq('user_id',user.id).in('role',['super_admin','question_editor']);if(configuredEvent)q=q.eq('event_id',configuredEvent);const{data:access,error:accessError}=await q.limit(1).maybeSingle();if(accessError){shell('Admin Console',`<p>${accessError.message}</p>`);return}if(!access){shell('Access denied','<p>This account does not have Admin Console privileges.</p>');return}appLoaded=true;root.innerHTML='';try{await import('./participants.tsx')}catch(e){appLoaded=false;shell('Admin failed to load',`<p>${e instanceof Error?e.message:String(e)}</p>`);return}addSignOut()})().finally(()=>{authInFlight=null});return authInFlight;
}
