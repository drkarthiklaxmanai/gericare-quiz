...
function renderLogin(message='Sign in with your authorized administrator email.'){
 ...
 form.onsubmit=async e=>{e.preventDefault();const email=(document.getElementById('email') as HTMLInputElement).value.trim();const status=document.getElementById('login-status')!;status.textContent='Sending…';const {error}=await supabase.auth.signInWithOtp({email,options:{emailRedirectTo:window.location.origin,shouldCreateUser:false}});status.textContent=error?error.message:'Check your email and open the sign-in link on this device.'};
}
