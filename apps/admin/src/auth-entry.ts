...
function addSignOut(){
 if(document.getElementById('gericare-signout'))return;
 const b=document.createElement('button');
 b.id='gericare-signout';
 b.textContent='Sign out';
 b.style.cssText='position:fixed;right:14px;bottom:14px;z-index:9999;border:1px solid #d8dee8;background:white;border-radius:10px;padding:9px 12px;font-weight:700;box-shadow:0 4px 18px rgba(15,23,42,.12)';
...
async function authorize(){
 ...
 authInFlight=(async()=>{
  try{
   ...
   if(!access){
    shell('Access denied','<p>This account is signed in but does not have Admin Console privileges.</p><button id="logout" style="padding:10px 14px;border:0;border-radius:9px;background:#172033;color:#fff;font-weight:700">Sign out</button>');
   ...
   try{
    ...
    shell('Admin failed to load',`<p style="color:#b91c1c">${msg}</p><button id="retry" style="padding:10px 14px;border:0;border-radius:9px;background:#172033;color:#fff;font-weight:700">Retry</button>`);
   ...
   appLoaded=false;
   const msg=e instanceof Error?e.message:String(e);
   shell('Admin Console',`<p>Unexpected error: ${msg}</p>`);
  }
