-- Final tie-break semantics: score first; then server time from round/final start
-- to the participant's last saved answer/revision. If there are no answers,
-- use elapsed time to finish/close (capped at the deadline).
...
declare v_a public.attempts; v_uid uuid:=auth.uid(); v_round_release timestamptz; v_score integer; v_time bigint; v_now timestamptz:=clock_timestamp(); v_last timestamptz;
begin
 ...
 select coalesce(sum(case when coalesce(is_void,false) then 0 else coalesce(points_awarded,0) end),0)::int,max(submitted_at) into v_score,v_last from public.responses where attempt_id=p_attempt;
 v_time:=greatest(0,extract(epoch from (coalesce(least(v_last,v_a.deadline_at),least(v_now,v_a.deadline_at))-v_a.started_at))*1000)::bigint;
...
begin
 ...
 update public.attempts a set score=coalesce((select sum(case when coalesce(r.is_void,false) then 0 else coalesce(r.points_awarded,0) end) from public.responses r where r.attempt_id=a.id),0)::int,
   valid_response_time_ms=greatest(0,extract(epoch from (coalesce((select least(max(r.submitted_at),a.deadline_at) from public.responses r where r.attempt_id=a.id),least(v_now,a.deadline_at))-a.started_at))*1000)::bigint,
   status='timed_out'::attempt_status,submitted_at=coalesce(a.submitted_at,v_now),updated_at=v_now
...
returns jsonb language plpgsql security definer set search_path='public','private' as $function$
declare v_f final_attempts%rowtype; v_uid uuid:=auth.uid(); v_status attempt_status; v_score integer; v_time bigint; v_now timestamptz:=clock_timestamp(); v_last timestamptz;
begin
 ...
 select coalesce(sum(coalesce(points_awarded,0)),0)::int,max(submitted_at) into v_score,v_last from final_responses where final_attempt_id=p_final_attempt;
 v_time:=greatest(0,extract(epoch from (coalesce(least(v_last,v_f.deadline_at),least(v_now,v_f.deadline_at))-v_f.started_at))*1000)::bigint;
 ...
 insert into audit_events(event_id,actor_user_id,event_type,action,metadata) select f.event_id,v_uid,'scoring','final_finished',jsonb_build_object('finalist_id',f.id,'score',v_f.score,'tiebreak_time_ms',v_f.valid_response_time_ms,'tiebreak_basis','last_final_answer') from finalists f where f.id=v_f.finalist_id;
...
returns jsonb language plpgsql security definer set search_path='public','private' as $function$
declare v_a public.attempts; v_count int; v_uid uuid:=auth.uid(); v_limit int; v_round_release timestamptz; v_score integer; v_time bigint; v_now timestamptz:=clock_timestamp(); v_last timestamptz;
begin
 select * into v_a from public.attempts where id=p_attempt for update;
 if v_a.id is null then raise exception 'attempt_not_found'; end if;
 if not exists(select 1 from public.event_participants ep where ep.id=v_a.participant_id and ep.user_id=v_uid) then raise exception 'forbidden'; end if;
 ...
 else update public.attempts set warning_count=v_count,updated_at=v_now where id=v_a.id; end if;
